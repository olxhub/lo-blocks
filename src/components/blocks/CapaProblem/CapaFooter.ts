...
import { baseAttributes, problemMixin, z_target, z_reduxStateKey } from '@/lib/blocks/attributeSchemas';
...
const CapaFooter = blocks.dev({
  ...
  attributes: baseAttributes.extend(problemMixin.shape).extend({
    target: z_target.optional().describe('Comma-separated grader IDs to trigger'),
