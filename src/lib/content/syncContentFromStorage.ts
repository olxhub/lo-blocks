...
export function getBlockVariant(blockId: string, locale: string): import('@/lib/types').OlxJson | null {
  const langMap = contentStore.blockIndex[blockId];
  return (langMap?.[locale] as import('@/lib/types').OlxJson) || null;
}

/**
 * Return the first non-autogenerated (human-authored) variant for a block.
 * Used to find the original source variant when starting from a translation.
 * Returns null if no variants exist or all are autogenerated.
 */
export function getOriginalVariant(blockId: string): import('@/lib/types').OlxJson | null {
  const langMap = contentStore.blockIndex[blockId];
  if (!langMap) return null;
  for (const variant of Object.values(langMap)) {
    const olxJson = variant as import('@/lib/types').OlxJson;
    if (!olxJson.autogenerated) return olxJson;
  }
  return null;
}
