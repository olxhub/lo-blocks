...
// Registry generator for blocks and CSS.
import fs from 'fs';
import path from 'path';
import { execSync } from 'child_process';
import { minimatch } from 'minimatch';

const blocksDir = path.resolve(process.cwd(), 'packages/shared/components/blocks/');
const ignoreFile = path.join(blocksDir, '.blockignore');
const blockI18nAutogenOutputFile = path.resolve(process.cwd(), 'packages/shared/lib/i18n/blockI18nAutogen.ts');
...
const registryTypes = {
  blocks: {
    ...
    outputFile: path.resolve(process.cwd(), 'packages/shared/components/blockRegistryAutogen.ts'),
    metadataFile: path.resolve(process.cwd(), 'packages/shared/components/blockMetadataAutogen.json'),
  ...
  css: {
    ...
    outputFile: path.resolve(process.cwd(), 'packages/shared/styles/components.css'),
...
function makeImportIdentifier(blockName, locale) {
...
function writeBlockI18nAutogen(allBlockMetadata = {}) {
  ...
  for (const [blockName, meta] of entries) {
    ...
    for (const [locale, resourceFile] of locales) {
      const importPath = toRelativeImportPath(
        blockI18nAutogenOutputFile,
        path.resolve(process.cwd(), resourceFile)
      );
      const importName = makeImportIdentifier(blockName, locale);
      imports.push(`import ${importName} from '${importPath}';`);
      registries.push(`  i18n.addResourceBundle('${locale}', '${namespace}', ${importName}, true, true);`);
    }
  }
  ...
  fs.writeFileSync(blockI18nAutogenOutputFile, body);
  console.log(`Generated block i18n autogen at ${blockI18nAutogenOutputFile}.`);
}
...
function generateRegistry(registryType, all = generateAllRegistryContents()) {
...
function main() {
  ...
  for (const registryType of Object.keys(registryTypes)) {
    generateRegistry(registryType, all);
  }

  writeBlockI18nAutogen(all.blocks?.metadata);
