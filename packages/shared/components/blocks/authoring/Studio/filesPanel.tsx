'use client';
// packages/shared/components/blocks/authoring/Studio/filesPanel.tsx
//
// Files sidebar panel: tree with dirty markers, per-file rename/delete menu
// (rename doubles as move — the input is pre-filled with the full path).
// Ported from apps/web/app/studio/panels/FilesPanel.tsx.
...
interface FilesPanelProps {
  ...
  /** Open the shared new-file dialog (owned by the shell — one creation flow). */
  onNewFile: () => void;
...
export function FilesPanel({
  ...
  // useState-ok: ephemeral inline-edit state — which file's ⋮ menu is open.
  const [fileActionPath, setFileActionPath] = useState<string | null>(null);
  // useState-ok: ephemeral inline-edit state — in-progress rename input text.
...
function FileTreeNode({
  ...
  // useState-ok: ephemeral hover/menu state — dir expansion, auto-open for depth < 2.
