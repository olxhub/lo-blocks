...
const CompactPopout = dev({
  ...
  attributes: z.object({
    ...
    target: z_reduxStateRef.optional().describe('Component ID to repoint (mode="target" only, e.g. "sidebar")'),
    targetContent: z_reduxStateRef.optional().describe('Block ID to display in the target (mode="target" only)'),
  }).strict(),
});
