// packages/shared/components/blocks/_test/_BadBlock.tsx
'use client';
 ...
 * ErrorNode). The parse-time behavior lives in BadBlock.ts; the
 * BadBlock*.olx fixtures pin one failure mode each.
 *
 ...
 */
export function _BadBlock(props: RuntimeProps) {
  const { throws = 'render', kind = 'native', message } = props;
  const msg = typeof message === 'string' && message
    ? message
    : 'BadBlock: deliberate render-time failure';
  ...
  if (throws === 'render') {
    ...
    if (kind === 'apperror') {
      ...
      const appError: AppError = { title: 'BadBlock', message: msg, technical: { kind, synthetic: true } };
      throw appError;
    }
    throw new Error(msg);
  }
