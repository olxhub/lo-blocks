...
import {
  source, version, path, withVersion, withoutVersion,
  withPath, makeAddress, hasVersion, scheme,
  toLofsAddress, toLofsContentPath, toLofsSourceLocator, toLofsVersionRef,
} from './address';

// Helper to avoid repeating toLofsAddress everywhere in tests
const addr = toLofsAddress;
const cp = toLofsContentPath;
const sl = toLofsSourceLocator;
const vr = toLofsVersionRef;
...
describe('address parsing', () => {
  ...
  describe('source-only address (no ://, no path)', () => {
    const a = addr('git@github.com:olxhub/lo-blocks.git');
  ...
  describe('git SSH @ not confused with version', () => {
    ...
    const a = addr('git@github.com:olxhub/lo-blocks.git://foo.olx');
  ...
  describe('version with dots and hyphens', () => {
    const a = addr('git@github.com:org/repo.git@v2.1-rc.3://foo.olx');
  ...
  describe('multiple :// — last one wins', () => {
    ...
    const a = addr('pg://school.edu/cs101://sub/dir/file.olx');
    it('path is after the last ://', () => {
      expect(path(a)).toBe('sub/dir/file.olx');
    });
...
describe('makeAddress', () => {
  it('source + path + version', () => {
    ...
    expect(path(a)).toBe('content/foo.olx');
  });
...
describe('withVersion', () => {
  it('adds version to unversioned address', () => {
    const a = addr('git@github.com:org/repo.git://foo.olx');
    const b = withVersion(a, vr('abc123'));
    expect(b).toBe('git@github.com:org/repo.git@abc123://foo.olx');
  });

  it('replaces existing version', () => {
    const a = addr('git@github.com:org/repo.git@main://foo.olx');
    const b = withVersion(a, vr('abc123'));
    expect(b).toBe('git@github.com:org/repo.git@abc123://foo.olx');
  });

  it('works on source-only address (no path)', () => {
    const a = addr('git@github.com:org/repo.git');
...
describe('withoutVersion', () => {
  ...
  it('no-op on unversioned address', () => {
    const a = addr('memory:session-42://draft.olx');
...
describe('hasVersion', () => {
  it('true for versioned address', () => {
    expect(hasVersion(addr('git@github.com:org/repo.git@main://foo.olx'))).toBe(true);
  });

  it('false for unversioned address', () => {
    expect(hasVersion(addr('git@github.com:org/repo.git://foo.olx'))).toBe(false);
  });
});

describe('scheme', () => {
  it('git SSH', () => {
    expect(scheme(addr('git@github.com:org/repo.git://foo.olx'))).toBe('git');
  });

  it('file', () => {
    expect(scheme(addr('file:/home/user/content://foo.olx'))).toBe('file');
  });

  it('pg', () => {
    expect(scheme(addr('pg://school.edu/cs101://hw1/p3.olx'))).toBe('pg');
  });

  it('memory', () => {
    expect(scheme(addr('memory:session-42://draft.olx'))).toBe('memory');
  });

  it('https git', () => {
    expect(scheme(addr('https://github.com/org/repo.git://foo.olx'))).toBe('https');
  });
});
