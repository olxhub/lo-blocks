...
import type { LofsCanonical, LofsRef, LofsVersion } from '../types/address';
import {
...
export function sourceSignature(input: {
  ...
  if (sourceKind === 'inline') {
    const path = toLofsContentPath(provenance || '_inline.olx');
  ...
  if (sourceKind === 'files') {
    ...
    const path = toLofsContentPath(provenance || '_files');
  ...
  return toLofsCanonical(makeAddress(origin, toLofsContentPath(id), toLofsVersion('preloaded')));
}
