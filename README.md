...
It being 2026 rather than 2011, and `react` existing, LO Blocks uses `react` directly, rather than (p)reinventing it poorly, as I did in 2011. We are also LLM-native.
...
A good place to navigate to is /docs/ -- that will demonstrate the capabilities of the system and breadth of activities available.
...
_**Aside**: XML has a bad reputation for bad reasons. It was designed to be a markup language, and HTML 2.0 (defined in SGML, a predecessor to XML) was actually quite easy-to-author. Elementary school students learned it. At some point, it started being used everywhere, [XML-RPC](https://en.wikipedia.org/wiki/XML-RPC), [SOAP](https://en.wikipedia.org/wiki/SOAP), [SAML](https://en.wikipedia.org/wiki/SAML), [ebXML](https://en.wikipedia.org/wiki/EbXML), etc. These attempt to encode *objects* in a *document language*, and naturally, the result was a train wreck. Ergo, we have JSON, YAML, TAML, and similar object languages which are much nicer for encoding objects and data structures. However, courses, assessments, and similar are *documents*, and it's possible to make very nice XML languages for describing them; here, the converse is true, and object languages (JSON, YAML, etc.) turn into a similar nightmare. OLX 1.0 was quite easy-to-author, and many MIT course teams continue to use it directly since it's much more pleasant to work with than a GUI. However, it too has a mixed reputation, as the Open edX OLX exporter is quite horrible and creates machine-generated gobbledygook._
...
1. **You can get your data out**. If you author a course in either Open edX or LO Blocks, and you have a conflict with your host, you can take your content elsewhere.
...
_... but with archeology, which proved prohibitively difficult for many uses in practice._
